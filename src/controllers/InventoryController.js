...
class InventoryController {
    ...
    async getInventoryByPlayerId(req, res) {
      try {
        const { playerId } = req.params;
        const inventory = await this.inventoryService.getInventoryByPlayerId(playerId);
        ...
        this.logger.error(`Error getting inventory for player ${req.params.playerId}: ${error.message}`);
    ...
    async removeItemFromInventory(req, res) {
      try {
        ...
        await this.inventoryService.removeItemFromInventory(playerId, itemId);
    ...
    async listInventoryItems(req, res) {
      try {
        const { playerId } = req.params;
        const items = await this.inventoryService.listInventoryItems(playerId);
    ...
    async equipItem(req, res) {
      try {
        const { playerId, itemId } = req.params;
        await this.inventoryService.equipItem(playerId, itemId);
    ...
    async unequipItem(req, res) {
      try {
        const { playerId, itemId } = req.params;
        await this.inventoryService.unequipItem(playerId, itemId);
    ...
    async sellItem(req, res) {
      try {
        ...
        await this.inventoryService.sellItem(playerId, itemId, price);
